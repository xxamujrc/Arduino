...
void loop()
{
    ...
    { // reset when less than 30RPM (dt>2s)
        cnt = measureCnt = 0;
        rpm = 0;
    }
    if (cnt == 1)
    ...
    if (cnt > 1 || !dispRotTime)
    { // keep previous time on the OLED until new rotation starts
        dispRotTime = rotTime;
        dispCnt = cnt;
    }
    if (rpm > maxrpm)
        maxrpm = rpm;
    sprintf(txt, "% 5u", dispCnt);
    oled.printStr(38, 0, txt);
    sprintf(txt, "% 5u", rpm);
    oled.printStr(38, 2, txt);
    sprintf(txt, "% 5u", maxrpm);
    oled.printStr(38, 4, txt);
    sprintf(txt, "% 5u", dispRotTime);
    oled.printStr(38, 6, txt);
}
